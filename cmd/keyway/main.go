...
Invoked bare with hook JSON on stdin, it's not meant to be run by hand —
that's what the commands below are for.

  keyway install [--write] [--project]    wire the hooks into settings.json
  keyway uninstall [--project]            remove exactly what install added
  keyway status [--project]               show what's actually registered
  keyway --version                        print the installed version
  keyway --help                           print this message

install prints what it would do and changes nothing unless --write is
given. --project targets ./.claude/settings.json instead of
~/.claude/settings.json. See https://github.com/justinstimatze/keyway.
`
...
func run(args []string, stdin *os.File, stdout io.Writer) {
	if len(args) > 1 {
		...
		case "--help", "-h":
			...
			return
		case "install":
			cmdInstall(args[2:], stdout)
			return
		case "uninstall":
			cmdUninstall(args[2:], stdout)
			return
		case "status":
			cmdStatus(args[2:], stdout)
			return
		}
	}
	if hookio.Disabled() {
